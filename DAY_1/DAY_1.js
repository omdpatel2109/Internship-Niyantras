...
//enter item and add to the list
const addItem = () => {
...
//event listeners to add item to the list
...
async function fetchData() {
    ...
    try{
        ...
        const cheapProducts = data.filter(item => item.price < 100);
        const titles = cheapProducts.map(item=> item.title);
        apiData.innerHTML = ""; //clear previous API data
        titles.forEach((title) => {
            const publicData = document.createElement('li');
            publicData.textContent = title;
